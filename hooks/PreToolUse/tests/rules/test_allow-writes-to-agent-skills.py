...
def test_match(rule):
    ...
    payload = _payload("Write", {"file_path": f"{HOME}/.agent-skills/.version-cache/fullstack-ts.json"})
...
def test_boundary_read_not_matched(rule):
    ...
    payload = _payload("Read", {"file_path": f"{HOME}/.agent-skills/.version-cache/fullstack-ts.json"})
    result = evaluate(payload, [rule], repo_root=REPO)
    assert result["decision"] == "proceed"
