...
function takenPorts(registry: Registry): Set<number> {
  ...
  for (const entry of Object.values(registry)) {
    for (const [key, val] of Object.entries(entry)) {
      if (key.endsWith("_PORT") && typeof val === "number") ports.add(val)
    }
  }
  return ports
}
...
async function main() {
  ...
  try {
    ...
    registry[projectName] = { API_PORT, DB_PORT, dir: root, updated: new Date().toISOString() }
    ...
    const envContent = [`API_PORT=${API_PORT}`, `DB_PORT=${DB_PORT}`, `DATABASE_URL=${DATABASE_URL}`].join(
      "\n",
    )

    writeFileSync(".env.ports", `${envContent}\n`)
    console.log("Ports written to .env.ports:", { API_PORT, DB_PORT, DATABASE_URL })
  } finally {
    release()
  }
}
